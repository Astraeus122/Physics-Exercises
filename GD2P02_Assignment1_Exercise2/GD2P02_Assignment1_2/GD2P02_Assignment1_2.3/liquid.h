...
public:
    ...
    Liquid(float x, float y, float width, float height)
    {
        ...
        shape.setFillColor(sf::Color(27, 120, 255, 100));  // Light blue with some transparency
    }
    ...
    {
        float dragCoefficient = 0.005f;  // Lowered coefficient
        sf::Vector2f velocity = sphere.velocity;
        float speed = std::sqrt(velocity.x * velocity.x + velocity.y * velocity.y);
        float dragMagnitude = dragCoefficient * speed * speed / (1 + speed * 0.1);  // Modified drag formula
